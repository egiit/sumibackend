export const QueryPackingIn = `SELECT (@rowid := @rowid +1) rowid, SA.PACK_BRCD_LINE SLINE, SC.LINE_PREF,  SC.LINE_UNIT, SC.LINE_NAME_SPV, SC.LINE_DESC, SB.PART_FCH_ACTUAL, SA.PACK_BRCD_PART SPART, SB.PART_NAME, SB.PART_UNIT, SC.LINE_NAME_SPV,
...
FROM (
  ...
  WHERE B.PACK_BRCD_DATE BETWEEN  :date AND  :date
  ...
  FROM (
	  SELECT
	   ...
	   IFNULL(C.OUT_PROD_QTY,0) QTY_PR
	   FROM plan_prod_daily_view_board A0 
		LEFT JOIN output_prod_good_hc B ON B.PLAN_ID = A0.OUT_PLAN_ID
	   LEFT JOIN output_prod_good_daily C ON C.OUT_PROD_DATE = :date AND C.OUT_PLAN_ID = A0.OUT_PLAN_ID
  ...
  CROSS JOIN (SELECT @rowid := 0) AS dummy 
