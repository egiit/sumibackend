...
export const QuerySummaryReport = `SELECT 
NB.OUT_PLAN_ID, NB.PLAN_PROD_DATE, NB.LINE_PREF, NB.LINE_ORDR, NB.LINE_CODE, 
NB.LINE_DESC, NB.LINE_NAME_SPV, NB.FCH, NB.OT_FLAG, NB.UNIT, NB.PLAN_PROD_CONSLD, NB.PARTN, NE.PART_NAME, NB.PROD_GROUP,
PLAN_HC, ACT_HC, NL_WH, NL_TARGET, ACT_TARGET, ACT_TARGET_JAM,
PROD_TIME_TOTAL,
NC.FIRST_TIME, NC.LAST_TIME, NC.FIRST_TIME_INPUT,
NC.LAST_TIME_INPUT,
count_actual_wh(NC.FIRST_TIME_INPUT, NC.LAST_TIME_INPUT, NB.PLAN_PROD_DATE) ACT_HOUR,  
TARGET_EH,
EH,
PLAN_AH,
ACT_HC*count_actual_wh(NC.FIRST_TIME_INPUT, NC.LAST_TIME_INPUT, NB.PLAN_PROD_DATE) ACTUAL_AH,
TARGET_EH/PLAN_AH TARGET_EFF,
EH/(ACT_HC*count_actual_wh(NC.FIRST_TIME_INPUT, NC.LAST_TIME_INPUT, NB.PLAN_PROD_DATE)) EFF,
NH.COUNT_GROUP,
NF.DEF_QTY,
NG.REJ_QTY,
ND.REMARK
FROM (	
	...
	PLAN_PROD_ORDR,
	SUM(PROD_TIME_TOTAL) PROD_TIME_TOTAL,
	#(SELECT MIN(OUT_PROD_TIME) LAST_TIME FROM output_prod_good S WHERE S.OUT_PLAN_ID = NA.OUT_PLAN_ID AND IFNULL(S.OUT_PROD_FLAG_DEL,'') <> 'D') FIRST_TIME,
	#(SELECT MAX(OUT_PROD_TIME) LAST_TIME FROM output_prod_good S WHERE S.OUT_PLAN_ID = NA.OUT_PLAN_ID AND IFNULL(S.OUT_PROD_FLAG_DEL,'') <> 'D') LAST_TIME,
	#(SELECT MIN(HOUR(OUT_PROD_TIME)) LAST_TIME FROM output_prod_good S WHERE S.OUT_PLAN_ID = NA.OUT_PLAN_ID AND IFNULL(S.OUT_PROD_FLAG_DEL,'') <> 'D') FIRST_TIME_INPUT,
	#(SELECT MAX(HOUR(OUT_PROD_TIME)) LAST_TIME FROM output_prod_good S WHERE S.OUT_PLAN_ID = NA.OUT_PLAN_ID AND IFNULL(S.OUT_PROD_FLAG_DEL,'') <> 'D') LAST_TIME_INPUT,
	(SUM(PROD_TIME_TOTAL)*FCH)/1000 EH, 
	(ACT_TARGET*FCH)/1000 TARGET_EH, 
	ACT_HC*NL_WH PLAN_AH
	FROM (
		...
		LEFT JOIN output_prod_good A ON A.OUT_PLAN_ID = B.OUT_PLAN_ID AND IFNULL(A.OUT_PROD_FLAG_DEL, TRUE) = TRUE
		LEFT JOIN output_prod_good_hc E ON E.PLAN_ID = B.OUT_PLAN_ID
		WHERE B.PLAN_PROD_DATE = :date AND C.LINE_PREF LIKE :unit #AND A.OUT_PROD_DATE='2022-05-17'
		GROUP BY	
			B.OUT_PLAN_ID,
			A.OUT_PROD_DATE,
			A.OUT_PROD_TIME, 
